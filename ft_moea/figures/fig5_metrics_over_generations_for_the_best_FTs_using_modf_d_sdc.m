...
check_folders = {'data_for_figures/fig4_5/MCS0TS0Acc1/';...
    'data_for_figures/fig4_5/MCS1TS1Acc1'};
saving_fig = 'Figures/';
save_plot = 0;
leg = {'d','sdc'};
...
%%
fig_position = [1233         450         213         190];
font_name = 'Arial Unicode MS';
text_size= 16;
linewidth = 2;

figure('color',[1 1 1],'position',fig_position), hold on
plot(1-compare{1}.acc_data,'marker','.','linestyle','-','linewidth',linewidth)
plot(1-compare{2}.acc_data,'marker','.','linestyle','-','linewidth',linewidth)
%plot([20 20],[0 0.23],'r-')
leg1 = legend(leg);
...
ylabel('\phi_d')
box on
set(gca,'FontName',font_name,'FontSize',text_size,'XTick',[0:10:40],'YTick',[0:0.1:0.3])
ylim([0 0.3])
if save_plot == 1
    h = gcf;
    set(h,'Units','Inches');
...
figure('color',[1 1 1],'position',fig_position), hold on
plot(1-compare{1}.acc_mcs,'marker','.','linestyle','-','linewidth',linewidth)
plot(1-compare{2}.acc_mcs,'marker','.','linestyle','-','linewidth',linewidth)
%plot([20 20],[0 0.6],'r-')
leg1 = legend(leg);
set(leg1,'location','best')
xlabel('Generation')
ylabel('\phi_c')
box on
set(gca,'FontName',font_name,'FontSize',text_size,'XTick',[0:10:40],'YTick',[0:0.2:0.6])
if save_plot == 1
    h = gcf;
    set(h,'Units','Inches');
    pos = get(h,'Position');
...
figure('color',[1 1 1],'position',fig_position), hold on
plot(compare{1}.tree_size,'marker','.','linestyle','-','linewidth',linewidth)
plot(compare{2}.tree_size,'marker','.','linestyle','-','linewidth',linewidth)
%plot([0 41],[24 24],'r-.')
leg1 = legend(leg);
set(leg1,'location','best')
xlabel('Generation')
ylabel('\phi_s')
box on
set(gca,'FontName',font_name,'FontSize',text_size,'YTick',[0:20:100],'XTick',[0:10:40])
if save_plot == 1
    h = gcf;
    set(h,'Units','Inches');
    pos = get(h,'Position');
...
figure('color',[1 1 1],'position',fig_position), hold on
plot(compare{1}.time/60,'marker','.','linestyle','-','linewidth',linewidth)
plot(compare{2}.time/60,'marker','.','linestyle','-','linewidth',linewidth)
%plot([20 20],[0 237.694],'r-')
leg1 = legend(leg);
set(leg1,'location','best')
xlabel('Generation')
ylabel('t (min)')
box on
set(gca,'FontName',font_name,'FontSize',text_size,'YTick',[0:50:250],'XTick',[0:10:40])
if save_plot == 1
    h = gcf;
    set(h,'Units','Inches');
    pos = get(h,'Position');
...
figure('color',[1 1 1],'position',fig_position), hold on
plot(diff(compare{1}.time/60),'marker','.','linestyle','-','linewidth',linewidth)
plot(diff(compare{2}.time/60),'marker','.','linestyle','-','linewidth',linewidth)
%plot([20 20],[0 10],'r-')
leg1 = legend(leg);
set(leg1,'location','best')
...
ylabel('\Delta_t (min)')
box on
set(gca,'FontName',font_name,'FontSize',text_size,'YTick',[0:2.5:10],'XTick',[0:10:40])
if save_plot == 1
    h = gcf;
    set(h,'Units','Inches');
    pos = get(h,'Position');
...
for j = 1 : length(c)
    a = c(j);
    pop_size  = a.pop_size;
    acc_data  = nan(a.pop_size,length(c.files));
    tree_size = nan(a.pop_size,length(c.files));
    time      = nan(a.pop_size,length(c.files));
    acc_mcs   = nan(a.pop_size,length(c.files));
    str_fts   = {};
    for i = 1 : length(a.files)
        b = load(strcat(['',a.path,'/',a.files{i},'']));
